...
public class EntryController extends BaseController {
	...
	public void init() {
		...
		/*
		 * Esto fue codigo necesario para cuando adicionamos los emoticones, reconvertir
		 * todas las entries anteriores de manera adecuada. Cuando se compruebe que los
		 * emoticones funcionan, hay que eliminarlo List<Entry> entries =
		 * entryDAO.objects("select e.eid as id,e.* from entries e", Entry.class); for
		 * (Entry e : entries) { preProcessEntry(e); entryDAO.updateEntry(e); }
		 */
	}
		...
		for (String token : tokenSet) {
			...
			if (token.startsWith("p#")) {
				if (NumberUtils.isNumber(token.substring(2))) {
					entryDAO.bool("select exists(select pid from problem where pid=?)",
							Integer.valueOf(token.substring(2)));
					text = text.replace(token, "<a target=\"new\" href=\"24h/problem.xhtml?pid=" + token.substring(2)
							+ "\" >" + token.substring(2) + "</a>");
				}
			} else if (token.startsWith("c#")) {
				if (NumberUtils.isNumber(token.substring(2))) {
					entryDAO.bool("select exists(select pid from contest where pid=?)",
							Integer.valueOf(token.substring(2)));
					text = text.replace(token, "<a target=\"new\" href=\"contest/contestview.xhtml?cid="
							+ token.substring(1) + "\" >" + token.substring(1) + "</a>");
				}
			} else if (token.startsWith("@")) {
				boolean reply = entryDAO.bool("select exists (select uid from users where username=?)",
						token.substring(1));
				entry.setReply(true);
				entry.setHasUsers(true);
				if (reply)
					text = text.replace(token, "<a target=\"new\" href=\"user/useraccount.xhtml?username="
	...
	public String addEntry(HttpServletRequest request, Principal principal, Locale locale, Model model,
			PagingOptions options, Entry entry, BindingResult errors, RedirectAttributes redirectAttributes) {
	...
	public String following(Model model, @PathVariable("username") String username,
	...
	public String pageFollowers(Model model, @PathVariable("username") String username,
			@RequestParam(required = false, value = "page", defaultValue = "1") Integer page) {
	...
	public String pageFollowing(Model model, @PathVariable("username") String username,
			@RequestParam(required = false, value = "page", defaultValue = "1") Integer page) {
	...
	public String n(Principal principal, @PathVariable("username") String username, Model model,
	...
	public String tables(Principal principal, @PathVariable("username") String username, Model model,
			PagingOptions options) {

		int uid = userDAO.idByUsername(username);

		IPaginatedList<Entry> entries = entryDAO.paginated("enabled.entries.list.by.user", Entry.class, 10, options,
				uid, uid);
	...
	public String tableEntries(Principal principal, Model model, PagingOptions options,
			@RequestParam(value = "entries", required = false, defaultValue = "users") String entryFilter) {
		...
		if ("users".equals(entryFilter) && principal != null)
			entries = entryDAO.paginated("enabled.entries.list", Entry.class, 10, options,
					userDAO.idByUsername(principal.getName()));
		else if ("following".equals(entryFilter) && principal != null)
			entries = entryDAO.paginated("select.followed.entries", Entry.class, 10, options,
					userDAO.idByUsername(principal.getName()));
		else if ("cojboard".equals(entryFilter) && principal != null)
			entries = entryDAO.paginated("select.cojboard.entries", Entry.class, 10, options,
					userDAO.idByUsername(principal.getName()));
	...
	@RequestMapping(produces = "application/json", value = "/user/like.json", method = RequestMethod.POST, headers = {
			"Accept=application/json" })
	public @ResponseBody String like(Principal principal, @RequestParam Integer id, Model model,
			PagingOptions options) {
	...
	@RequestMapping(produces = "application/json", value = "/user/dislike.json", method = RequestMethod.POST, headers = {
			"Accept=application/json" })
	public @ResponseBody String dislike(Principal principal, @RequestParam Integer id, Model model,
			PagingOptions options) {
	...
	@RequestMapping(produces = "application/json", value = "/admin/disableentry.json", method = RequestMethod.POST, headers = {
			"Accept=application/json" })
	public @ResponseBody Entry disableEntry(HttpServletRequest request, Locale locale, Principal principal,
	...
	public void manageEntries(Entry entry, HttpServletResponse response, Locale locale, Principal principal,
			Model model) {
