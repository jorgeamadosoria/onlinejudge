...
public class UserController extends BaseController {
	...
	private static final long serialVersionUID = -4497643675276940605L;
	@Resource
	...
	public String userAccount(HttpServletRequest request, Model model, Principal principal,
		...
		model.addAttribute("currentUsername",
				principal != null && user != null ? principal.getName().equals(user.getUsername()) : false);
		...
		if (principal != null) {
			...
			if (pUid == uid) {
				...
				model.addAttribute("isfollowing",
						userDAO.bool("user.is.following", uid, userDAO.idByUsername(principal.getName())));
			}
		} else {
			model.addAttribute("isfollowing", null);
		}
	 ...
	 * public String compareUsers(Principal principal, Model
	 * model, @RequestParam(required = false, value = "uid1") String
	 * user1, @RequestParam(required = false, value = "uid2") String user2) {
	 * model.addAttribute("compare", new CompareUsers("", ""));
	 * model.addAttribute("error", true); return "/user/compareusers"; }
	 */

	@Deprecated
	@RequestMapping(value = "/compareusers.xhtml", method = RequestMethod.GET)
	public String compareUsersPost(Principal principal, Model model,
			@RequestParam(required = false, value = "uid1") String user1,
			@RequestParam(required = false, value = "uid2") String user2) {
		int error = 1;
	...
	public String compareUsersByTag(Principal principal, Model model,
		...
		List<ProblemClassification> classifications = problemDAO.objects("problem.classifications",
		...
		if (!error) {
			if (idClassification != -1) {
				// Problems solved
				Set<ProblemComplexity> solved_user1 = new TreeSet<ProblemComplexity>(
						userDAO.getPublicProblemsSolvedByUIdAndTagId(uid1, idClassification));
				Set<ProblemComplexity> solved_user2 = new TreeSet<ProblemComplexity>(
						userDAO.getPublicProblemsSolvedByUIdAndTagId(uid2, idClassification));
				...
				// Problems tried
				Set<ProblemComplexity> tried_user1 = new TreeSet<ProblemComplexity>(
						userDAO.getPublicProblemsTriedByUIdAndTagId(uid1, idClassification));
				Set<ProblemComplexity> tried_user2 = new TreeSet<ProblemComplexity>(
						userDAO.getPublicProblemsTriedByUIdAndTagId(uid2, idClassification));
				...
				Set<ProblemRichTitle> allSolved_user1 = new TreeSet<ProblemRichTitle>(
						userDAO.getPublicProblemsSolvedWithRichTitleByUid(uid1));
				Set<ProblemRichTitle> allSolved_user2 = new TreeSet<ProblemRichTitle>(
						userDAO.getPublicProblemsSolvedWithRichTitleByUid(uid2));
				...
				Set<ProblemRichTitle> allTried_user1 = new TreeSet<ProblemRichTitle>(
						userDAO.getPublicProblemsTriedWithRichTitleByUid(uid1));
				Set<ProblemRichTitle> allTried_user2 = new TreeSet<ProblemRichTitle>(
						userDAO.getPublicProblemsTriedWithRichTitleByUid(uid2));
		...
		model.addAttribute("error", error); // FIXME: hacer mas particular y pasar error de parametros incorrectos al
											// formlario
	...
	public String updateAccount(SecurityContextHolderAwareRequestWrapper requestWrapper, Model model,
			Principal principal, User user, @RequestParam(value = "avatar", required = false) MultipartFile avatar,
	...
	public String forgottenPassword(Model model, Principal principal,
	...
	public String forgottenPassword(Model model, User user, java.util.Locale locale, BindingResult bindingResult,
		...
		if (this.passcode != null) {
			...
			ValidationUtils.rejectIfEmptyOrWhitespace(bindingResult, "confirmPassword",
					"judge.register.error.password");
			...
			String subject = messageSource.getMessage("mail.pass.recover.subject", new Object[0],
	...
	public String forgottenSuccess(Model model,
			@RequestParam(required = false, value = "change", defaultValue = "0") Integer change) {
