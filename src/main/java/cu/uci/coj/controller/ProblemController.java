...
public class ProblemController extends BaseController {
    ...
    public String Problem(HttpServletRequest request, Locale locale, Model model, Principal principal,
        ...
        if (request.isUserInRole(Roles.ROLE_USER)) {
            ...
            if (view_pinfo) {
                model.addAttribute("classifications",
                        problemDAO.objects("get.problem.classifications", ProblemClassification.class, problem.getPid()));
            }
        }
    ...
    public String problems(Model model, Locale locale, Principal principal, PagingOptions options,
    ...
    public String tablesProblems(Model model, Locale locale, Principal principal, PagingOptions options,
            @RequestParam(required = false, value = "pattern") String pattern,
            @RequestParam(required = false, defaultValue = "0", value = "filterby") Integer filterby,
            @RequestParam(required = false, defaultValue = "-1", value = "classification") Integer idClassification,
            @RequestParam(required = false, defaultValue = "-1", value = "complexity") Integer complexity) {
        ...
        if (found != 0) {
            IPaginatedList<Problem> problems = problemDAO.findAllProblems(locale.getLanguage(), pattern, found, options, username,
                    username == null ? 0 : problemDAO.integer("select.uid.by.username", username), filterby, idClassification,
                    complexity);
