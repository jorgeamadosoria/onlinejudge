...
<div class="post-inner article">
		<div th:if="${contest != null and contest.initdate != null and (contest.past or contest.running or contest.coming)}" class="pull-right">
			<table class="contestlanguages" style="align: right">
				<tbody>
					<tr>
						<td><b th:text="|#{fieldhdr.status}:|" ></b></td>
						<td><span th:if="${contest.running == true}"
							class="label label-success" th:text="#{fieldhdr.running}"></span>
							<span th:if="${contest.coming == true}"
							class="label label-danger" th:text="#{fieldhdr.coming}"></span> <span
							test="${contest.past == true}" th:text="#{fieldhdr.past}"></span>
						</td>
		...
		<table class="navigating">
			<tbody>
				<tr>
					...
					<td><a
						href="<c:url value="/contest/myclarifications.xhtml?cid=${contest.cid}"/>"><img
							src="<c:url value="/images/clarifications.png"/>" alt="" /> <spring:message
								code="link.clarifications" /> <c:if test="${totalmsg != 0}">(${totalmsg})</c:if>
							<c:if test="${unread != 0}">
								<span class="label label-danger">(${unread})</span>
							</c:if> <c:if test="${isjudge == true and unreply ne 0}">
		...
		<c:choose>
			<c:when test="${contest.repointing == true}">
				<div class="label label-info">
					<i class="fa fa-info-circle"></i> The contest its been repointed
						...
						<c:choose>
								...
								<c:choose>
									<c:when test="${group == true}">
										<c:forEach items="${groups}" var="igroup">
											<div style="clear: both; float: right">
												<a
													href="<c:url value="/contest/cscoreboard.xhtml?cid=${contest.cid}" />">
											...
											<table class="ACM">
												<thead>
													<th class="headrk"><spring:message
															code="tablehdr.rank" /></th>
													<th colspan="3" class="team" style="text-align: center;">
														<spring:message code="tablehdr.contestant" />
													</th>
													<th class="headacc"><spring:message code="tablehdr.ac" />
													</th>
													<th class="headtime"><spring:message
															code="tablehdr.time" /></th>
													<c:forEach items="${problems}" var="problem">
														<th width="6%"><a class="linkheader"
												...
												<c:forEach items="${igroup.users}" var="user">
													<tr>
														...
														<td class="team"><a title="${user.username}"
														...
														<td><a
															href="<c:url value="/contest/cstatus.xhtml?cid=${contest.cid}&status=ac&username=${user.username}"/>">${user.acc}</a>
														</td>
														<td>${user.penalty}</td>
									...
									<c:otherwise>
										<c:if test="${not empty users}">
											<div style="clear: both; float: right">
												<a
													href="<c:url value="/contest/cscoreboard.xhtml?cid=${contest.cid}&groupby" />">
										...
										<table class="ACM" border="1px">
											<thead>
												<th class="headrk"><spring:message code="tablehdr.rank" />
												</th>
												<th colspan="3" class="team" style="text-align: center;">
													<spring:message code="tablehdr.contestant" />
												</th>
							...
							<c:when test="${contest.style eq 3}">
								...
								<div id="display-table-container"
									data-reload-url="/tables/globalcontestscoreboard.xhtml"></div>
